...
import {
    inicializarMercado, toggleCamposInversion, ejecutarInversion,
    retirarInversion, ejecutarEnvioAhorro, ingresarSaldoExistente
} from './billetera.js';
...
import { inicializarSelectorHistorico, cambiarPestaña, actualizarApp } from './render.js';
...
async function resetearApp() {
    if(await mostrarConfirmacion("⚠️ PELIGRO CRÍTICO: Se purgarán todos los balances de la nube.", {peligroso: true})) {
        if(await mostrarPrompt("Escribe BORRAR:") === "BORRAR") {
            estadoApp.todosLosMovimientos = []; estadoApp.suscripciones = []; estadoApp.patrimonio = { pesos: 0, dolares: 0 }; estadoApp.inversiones = []; estadoApp.listaAmigos = [];
